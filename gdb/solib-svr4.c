...
svr4_current_sos_direct (struct svr4_info *info)
{
  ...
  JPRINTF("Debug base: 0x%llx\n", info->debug_base);
  char *jr_debug = getenv("JDEBUG_R_DEBUG");
  if (jr_debug) {
    info->debug_base = (CORE_ADDR)strtoul(jr_debug, (char**)0, 0);
    JPRINTF("WARNING: Using user defined debug base: 0x%llx\n", info->debug_base);
  } else {
